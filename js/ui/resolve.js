...
export function renderSecretUnlock(unlock, tones){
  ...
  const activeTones = tones || unlock?.tones || unlock?.secret?.combo || [];

  $('scr-secret').innerHTML = `
    <div class="center" style="margin-top:20px">
      <div class="sc" style="color:#c9b3de;letter-spacing:.35em;font-size:.85rem">THE TONES ALIGN — ${activeTones.map(t=>t.toUpperCase()).join(', ')}</div>
